...
body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif;
       background: #0f1419; color: #e7ecef; min-height: 100vh; }
a { color: inherit; text-decoration: none; }
.header { border-bottom: 1px solid #2a3a4e; padding: 20px 40px;
          display: flex; align-items: center; justify-content: space-between; }
.header-brand { display: flex; align-items: center; gap: 10px; }
.live-dot { width: 7px; height: 7px; border-radius: 50%; background: #6096ba;
            flex-shrink: 0; animation: pulse 2.5s ease-in-out infinite; }
@keyframes pulse { 0%, 100% { opacity: 1; } 50% { opacity: 0.25; } }
.header h1 { font-size: 18px; font-weight: 600; letter-spacing: -0.3px; }
.header p { font-size: 13px; color: #8faabe; margin-top: 2px; }
.container { max-width: 1100px; margin: 0 auto; padding: 40px 24px; }
.stats-row { display: grid; grid-template-columns: repeat(auto-fit, minmax(160px, 1fr));
             gap: 16px; margin-bottom: 48px; }
.stat-card { background: #1a2332; border: 1px solid #2a3a4e; border-radius: 10px; padding: 20px; }
.stat-card .label { font-size: 11px; color: #8faabe; text-transform: uppercase;
...
.stat-card .sub { font-size: 12px; color: #6080a0; margin-top: 4px; }
.section-title { font-size: 11px; font-weight: 600; text-transform: uppercase;
                 letter-spacing: 0.8px; color: #8faabe; margin-bottom: 16px;
                 border-left: 2px solid #274c77; padding-left: 10px; }
...
.metric { }
.metric .m-label { font-size: 11px; color: #8faabe; text-transform: uppercase;
                   letter-spacing: 0.4px; margin-bottom: 4px; }
.metric .m-value { font-size: 20px; font-weight: 700; letter-spacing: -0.5px;
                   font-family: ui-monospace, Menlo, 'Courier New', monospace; }
.metric .m-sub { font-size: 11px; color: #6080a0; margin-top: 2px; }
...
.badge { display: inline-flex; align-items: center; gap: 5px; font-size: 11px;
         padding: 5px 10px; border-radius: 20px; border: 1px solid #2a3a4e;
         color: #6080a0; cursor: default; }
.badge.yt { border-color: #2a3a4e; color: #a3cef1; background: rgba(96,150,186,0.1); }
...
.perf-table th { font-size: 11px; text-transform: uppercase; letter-spacing: 0.5px;
                 color: #8faabe; text-align: left; padding: 10px 12px;
                 border-bottom: 1px solid #2a3a4e; background: #141c28; }
.perf-table td { font-size: 13px; padding: 10px 12px; border-bottom: 1px solid #1e2c3e;
                 color: #c8d8e8; font-family: ui-monospace, Menlo, 'Courier New', monospace; }
.perf-table td.label-col { font-family: inherit; color: #8faabe; font-size: 12px; }
.perf-table tr:nth-child(even) td { background: #131b27; }
.perf-table tr:hover td { background: #1e2c3e; }
.view-bar-wrap { display: flex; align-items: center; gap: 10px; }
.view-bar { height: 3px; background: #2a3a4e; border-radius: 2px; flex: 1; max-width: 80px; }
.view-bar-fill { height: 3px; background: #6096ba; border-radius: 2px; }
.back { font-size: 13px; color: #6080a0; margin-bottom: 24px; display: inline-block; }
.back:hover { color: #a3cef1; }
.divider { border: none; border-top: 1px solid #2a3a4e; margin: 40px 0; }
.experiment-note { font-size: 12px; color: #4a6a8a; margin-top: 40px; padding-top: 20px;
